...
%addpath('C:\Documents and Settings\nn\Escritorio\ZZ_gerardo\Funciones')  %adaptarlo al directorio que usemoes en la UNS
...
function [DATA,init,cond_salida]=runtrial(DATA,w,init,trial)  
    ...
    t=GetSecs;Screen('Flip', w,t+init.ifi); 
    str=['PrimeraLetra ' num2str(trial)];if eyelink('isconnected');Eyelink('Message', str);end  
    ...
    t=GetSecs;Screen('Flip', w,t+init.ifi); 
    Screen('close',temp);
    str=['EmpiezaLectura ' num2str(trial)];if eyelink('isconnected');Eyelink('Message', str);end
    ...
    str=['FinLectura ' num2str(trial)];if eyelink('isconnected');Eyelink('Message', str);end
    ...
    str=['FinTrial ' num2str(trial)];if eyelink('isconnected');Eyelink('Message', str);end  
...
try
    ...
    str=['EmpiezaPregunta ' num2str(trial)];if eyelink('isconnected');Eyelink('Message', str);end
...
function pseudo_calibracion_old(window,init)   
    xx=[.2 .5 .8 .2 .5 .8 .2 .5 .8]*init.width;
    yy=[.2 .2 .2 .5 .5 .5 .8 .8 .8]*init.height;
    
    Screen('fillrect',window,init.colback)
    Screen('TextSize', window, 20);%ver tamaño    
    Screen('DrawText', window, 'Mire los puntitos rojos que aparecen',init.CX-250,init.CY-100,0);
    Screen('DrawText', window, 'Pulse una tecla para comenzar ',init.CX-200,init.CY,0);
    t=GetSecs;Screen('Flip', window,t+init.ifi); 
    while ~KbCheck;end %espero a que toque una tecla
    while KbCheck;end %espero a que la suelte
    WaitSecs(.5);  
    ...
    for i=1:length(xx)      
        size=5;
        poscirculo=[xx(i)-size yy(i)-size xx(i)+size yy(i)+size];       
        Screen('FillRect', window,init.colback);
        Screen('FillOval', window,[255 0 0], poscirculo);
        t=GetSecs;Screen('Flip', window,t+init.ifi); 
        str=['pseudocalib ' num2str(xx(i)) ',' num2str(yy(i)) ];if eyelink('isconnected');Eyelink('Message', str);end  
        [tresp condicion]=espera_posicion_ojo([xx(i) yy(i)],30,0);%espero a que el ojo este en donde la primera letra
        WaitSecs(0.3);
    end
end
