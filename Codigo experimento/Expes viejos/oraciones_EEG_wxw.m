...
% Inicio trial      EmpiezaLectura #trial   #trial: 1 a 120
% cruz de fijacion	CruzFijacion #trial     #trial: 1 a 120
% blanco            Blanco #tipo            150 + #tipo: 150-152
% Onset palabra     Trial:#trial|PalabraOn:#indpal  210+#indpal: 211-225
% Offset palabra    Trial:#trial|PalabraOff:#indpal 230+#indpal: 231-245
...
% Numero de palabra: 230+numpalabra, 231:245


try 
   
addpath('j:\EyeTracker\Funciones')  %adaptarlo al directorio que usemoes en la UNS
    ...
    init.TiempoPalabra  = 0.25; %en segundos
    init.SOA            = 0.70; %en segundos
    init.cross          = 0.50; %en segundos
    init.delay          = 0.50; %en segundos
    init.blink          = 2.00; %en segundos
    init.delayblink     = 1.00; %en segundos
    
    init.ntrial_descanso=31;
...
%  preguntas iniciales
    respuesta= inputdlg({'Nombre (no más de 8 caracteres)' '¿Usamos Eyelink?(S/N)'},'Ingrese su Nombre',1,{'test' 'N'});
...
% PRÁCTICA!!!!!!
    ...
    Screen('textstyle',w, init.fontstyle);%0=normal,1=bold,2=italic,4=underline,8=outline,32=condense,64=extend.                   
...
% EXPERIMENTO!!!!!!
    if cond_salida_pract==0 %solo hago el experimento si no abortaron la practica
        ...
        for trial=1:length(DATA) %corre los trials
            ...
            if mod(trial,init.ntrial_descanso)==0
...
% Catch errores
    ...
    ShowCursor
    if EYELINKCONECTADO
        eyelink_receive_file(filename) %recibe file de datos *.edfo 
    end
    save porsi DATA init DATAPRACT
    ListenChar(0)
...
%     drawfix(w,init);
%     t=GetSecs;Screen('Flip', w,t+init.ifi);             
% %     keyCode=espero_mouse_teclado();%espero hasta que toque una tecla
...
%         end
%     end        
%     WaitSecs(0.1);%para que al menos este apagada por 100ms
    ...
    Screen('FillRect', w, init.colback);
    t=GetSecs;Screen('Flip', w,t+init.ifi);     
    str=['Blanco ' num2str(DATA(trial).tipo)];
    if Eyelink('isconnected');Eyelink('Message', str);end  
    condicion=150+DATA(trial).tipo;
    escribo_valor_puerto_paralelo(init,condicion)
    WaitSecs(init.delay);
...
%     [tresp condicion]=espera_posicion_ojo([cx cy],10,0);
    ...
    palabras = textscan(DATA(trial).oracion,'%s','delimiter',' ');
    palabras=palabras{1};
    DATA(trial).palabras =palabras ;
    for indpal=1:length(palabras)
        palabra=palabras{indpal};
        Screen('fillrect',w,init.colback);
        Screen('TextFont', w, 'Courier New');
        Screen('textstyle',w, init.fontstyle);%0=normal,1=bold,2=italic,4=underline,8=outline,32=condense,64=extend.            
        Screen('TextSize', w, init.fontsize);%ver tamaño    
        DrawFormattedText(w, palabra,'center','center',init.coltext);
        t=GetSecs;Screen('Flip', w,t+init.ifi); 
        str=['Trial:' num2str(trial) '|PalabraOn:' num2str(indpal)];
        if Eyelink('isconnected');Eyelink('Message', str);end
        condicion=210+indpal;
        escribo_valor_puerto_paralelo(init,condicion);
        WaitSecs(init.TiempoPalabra);
        
        Screen('fillrect',w,init.colback);
        t=GetSecs;Screen('Flip', w,t+init.ifi); 
        str=['Trial:' num2str(trial) '|PalabraOff:' num2str(indpal)];
        if Eyelink('isconnected');Eyelink('Message', str);end
        condicion=230+indpal;
        escribo_valor_puerto_paralelo(init,condicion);
        WaitSecs(init.SOA - init.TiempoPalabra);
    end
...
%     t=GetSecs;Screen('Flip', w,t+init.ifi);     
    ...
    if DATA(trial).preguntasino==1
        ...
        Screen('FillRect', w, init.colback);
        t=GetSecs;Screen('Flip', w,t+init.ifi);     
        WaitSecs(init.delayblink);
        
        %cruz de fijacion al principio
        Screen('FillRect', w, init.colback);
        Screen('FillOval', w, [0 255 0], [init.CX init.CY init.CX init.CY]+[-15 -15 15 15]);
        t=GetSecs;Screen('Flip', w,t+init.ifi);     
        keyCode=espero_mouse_teclado(init.blink);%espero hasta que toque una tecla
        %si aprieta esc, saldra
        %si aprieta C, recalibra            
        espero_suelte_mouse_teclado;
        if find(keyCode)==27 % si toca esc, termino
