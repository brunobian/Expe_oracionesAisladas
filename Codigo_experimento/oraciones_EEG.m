...
% falta. Muchas se pueden dejar, total derían estar dentro de un if EEG que
% se inicializa con el prompt que aparece al lanzar la función
...
%     DATA=Shuffle(DATA);%#ok<NODEF> % revuelve las oraciones.
    ...
    disp('Archivo de pr�ctica "oracionesPRACTICA" cargado.')
    DATAPRACT=load('oracionesPRACTICA');DATAPRACT=DATAPRACT.DATA;
    DATAPRACT=agrega_preguntasino(DATAPRACT,.5);
%     
%  preguntas iniciales
    respuesta= inputdlg({'Nombre (no m�s de 8 caracteres)' ...
                         'Edad' ...
                         '�Usamos Eyelink?(S/N)' ...
                         '�Usamos EEG?(S/N)' ...
                         'Inicio'},...
    ...
    if init.etConectado 
        ...
        disp('Modo prueba, sin conexi�n con el Eyetracker.')        
    end
...
%         Eyelink('command','heuristic_filter=1 1'); %setea filtro STD para ver microsacadas
    ...
    init.baseCode  = 0; % C�digo base. Del que parte y al que vuelve dsp de cada marca
...
% PR�CTICA!!!!!!
    ...
    Screen('DrawText', w, 'Pulse una tecla para comenzar pr�ctica',init.CX-300,init.CY-100,0);
    ...
    for trial=1:length(DATAPRACT)
        if start  == 1 % Si estamos corriendolo por primera vez
            ...
            if cond_salida_pract==1
                disp(['Pr�ctica interrumpida en el trial ' num2str(trial) '.'])            
                break
            end
...
% EXPERIMENTO!!!!!!
    if cond_salida_pract==0 %solo hago el experimento si no abortaron la practica
        ...
        Screen('fillrect',w,init.colback)
        Screen('TextSize', w, 20);%ver tama�o    
        Screen('DrawText', w, 'Fin de la pr�ctica',init.CX-200,init.CY-100,0);
        ...
        for trial=start:length(DATA) %corre los trials desde el idicado en el prompt
            ...
            if mod(trial,42)==0
                ...
                instrucciones={'Vamos a hacer una pausa para descansar.' '' 'Pulse una tecla cuando est� listo para seguir.'};
    ...
    % Env�o marcas del Final del Experimento
    eegMsg = 255;
    etMsg  = ['keyword ' num2str(eegMsg)];
    sendTriggers(init, eegMsg, etMsg)
...
function [DATA,init,cond_salida]=runtrial(DATA,w,init,trial)  
    ...
    % Env�o marcas del Comienzo de lectura
    ...
    % Env�o marcas del Fin de lectura
    eegMsg = 221;
    etMsg  = ['keyword ' num2str(eegMsg)];
    sendTriggers(init, eegMsg, etMsg)
...
try
    ...
    % Env�o marcas del Comienzo Pregunta
    eegMsg = 222;
    etMsg  = ['keyword ' num2str(eegMsg)];
    sendTriggers(init, eegMsg, etMsg)
...
% muestra instrucciones en la pantalla   
    ...
    Screen('TextSize', w, 18);%ver tama�o            
    Screen('FillRect', w, init.colback);    
